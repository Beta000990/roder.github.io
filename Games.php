...
foreach ($stmt as $row) {
    ...
    $creatorq->bindValue(':creator', $row['creator_id'], PDO::PARAM_INT);
    $creatorq->execute();
    ...
    if ($row['players'] > 0) {
        echo '<div class="GameCurrentPlayers"><span class="DetailHighlighted">' . htmlspecialchars($row['players']) . ' players online</span></div>';
    }
